...
Run `python -B scripts/build.py`. The builder verifies the local game fingerprints, runs the synthetic profile, collision, snapshot, motion, settlement, completion, loader and package checks, and writes `releases/Enemy-Collision-Synchronized-v2.6.zip`. It does not launch or install into the game.
